...
class GPT(nn.Module):
    ...
    def __init__(self, config: GPTConfig):
        ...
        modules.append(EmbeddingStem(config))
        # transformer
        for _ in range(config.n_layer):
            modules.append(Block(config))
        # decoder head
        modules.append(nn.LayerNorm(config.n_embd))
        modules.append(nn.Linear(config.n_embd, config.vocab_size, bias=False))

        self.model = nn.Sequential(*modules)
        self.model = Pipe(self.model, chunks=config.gpus, checkpoint='never') # no checkpointing due to compatibility with DDP
